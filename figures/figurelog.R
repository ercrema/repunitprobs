...
## Plot
par(mar=c(4, 4, 2, 5)) #c(bottom, left, top, right)
par(xaxs="i")
par(yaxs="i")
plotymax <- ymax <- max(southgreece$Sites)*1.1
plot(southgreece$YearBCE, southgreece$Sites, type="l", col="white", cex.axis=0.8, xlim=c(-5505,-2200), ylim=c(0,plotymax), xlab="",ylab="",axes=FALSE)
mtext(side=1,line=2.3,text = "Year BC")
mtext(side=2,line=2.3,text = "Number of Sites")
polygon(c(southgreece$YearBCE,rev(southgreece$YearBCE)),c(southgreece$Sites,rep(0,length(southgreece$Sites))), col="lightblue", border=NA)
axis(1,at=seq(-5500,-2500,500),labels=seq(5500,2500,-500))
axis(2,at=seq(0,150,50),pos=c(-5550))

par(new=TRUE)
plot(southgreece$YearBCE,southgreece$SPD,type='n',xlim=c(-5505,-2200),lwd=2,axes=F,xlab='',ylab='',ylim=c(0,max(southgreece$SPD)))
polygon(c(southgreece$YearBCE,rev(southgreece$YearBCE)), c(southgreece$SPD,rep(0,length(southgreece$SPD))), col=rgb(255,140,0,100,maxColorValue = 255), border=NA)
axis(side=4,at=seq(0,0.15,0.05))
mtext(side=4,line=2.3,text = "Summed Probability",las=3)


lines(c(-5500,-5500),c(0.15,0.17), lwd=2)
lines(c(-4500,-4500),c(0.15,0.17), lwd=2)
text(x=-5000, y=0.16, labels="Late\nNeolithic", font=4, cex=0.7)
text(x=-3850, y=0.16, labels="Final\nNeolithic", font=4, cex=0.7)
lines(c(-3200,-3200),c(0.15,0.17), lwd=2)
text(x=-2600, y=0.16, labels="Early\nBronze Age", font=4, cex=0.7)
mtext("A", side=1, font=2, cex=1.2, line=2.2, adj=-0.08)
...
#### Plot
par(mar=c(4, 3, 0.2, 0.2)) #c(bottom, left, top, right)
...
mtext(side=1,line=7,"Ceramic Phases",cex=1)
axis(1,at=1:8,labels=levels(chiba$SimplifiedPhase),las=2)
legend(6,280,legend=c("Ariyoshi-Kita","Aioi","Other Sites"),fill=c("orange","royalblue","grey"),cex=1)
mtext("D", side=1, font=2, cex=1.2, line=1, adj=-0.05)
...
pdf(file = "./figure4.pdf",width = 7.2,height = 4.8)
layout(matrix(c(1,2,1,3,1,4),nrow=2,ncol=3))
par(mar=c(4, 4, 1, 1)) #c(bottom, left, top, right)
plot(simyears, mediankd, type="l",  xlim=c(-1750,-751), xlab="Years BC", ylab="",axes=FALSE)
...
legend("topright", lwd=c(3,1), col=c("plum1","grey25"), legend=c("Early mines (c1700 BC)","mean (early/middle/late)"), bty="n", cex=0.7)
...
lines(pooled[,1],pooled[,2], col="grey25", lwd=2)
legend("topright", lwd=c(3,1), col=c("cadetblue","grey25"), legend=c("Middle mines (c1300 BC)","mean (early/middle/late)"), bty="n", cex=0.7)
...
lines(pooled[,1],pooled[,2], col="grey25", lwd=2)
legend("topright", lwd=c(3,1), col=c("palegreen","grey25"), legend=c("Late mines (c900 BC)","mean (early/middle/late)"), bty="n", cex=0.7)
...
text(x=-0.30,y=-0.05,labels = "Nucleation (1st period)",cex=0.5)
